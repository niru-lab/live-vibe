...
import { Mail, MapPin, Calendar, Check, ChevronRight } from 'lucide-react';
        ...
        <div className="overflow-y-auto max-h-[calc(80vh-100px)] space-y-3 pb-20">
          {isLoading ? (
            ...
            messages.map((message) => (
              <button
                ...
                className={`w-full text-left glass rounded-2xl p-4 transition-all hover:scale-[1.02] ${
                  !message.is_read ? 'ring-2 ring-primary/50 bg-primary/5' : ''
                }`}
              >
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full glass neon-glow-sm">
                <Mail className="h-10 w-10 text-muted-foreground" />
              </div>
              <h3 className="font-semibold text-foreground mb-1">Keine Nachrichten</h3>
              <p className="text-sm text-muted-foreground max-w-xs">
                Wenn du für Events akzeptiert wirst, erhältst du hier Nachrichten mit Details.
              </p>
            </div>
          )}
        </div>
...
// Export a button component to trigger the inbox
