...
import { Check, X, Send, MapPin, Users, Clock, MessageCircle } from 'lucide-react';
...
export function AttendeeManager({ 
  ...
  const handleAccept = async (attendeeId: string, userId: string) => {
    try {
      await hostAction.mutateAsync({ attendeeId, accepted: true });
      setSelectedAttendee(userId);
      setMessageContent(`Hey! 🎉 Du bist für "${eventName}" akzeptiert!\n\n${includeAddress ? `📍 Adresse: ${eventAddress}\n\n` : ''}Bis bald!`);
      toast.success('Gast akzeptiert!');
    } catch (error) {
  ...
  const handleReject = async (attendeeId: string) => {
    try {
      await hostAction.mutateAsync({ 
        attendeeId, 
        accepted: false,
        message: 'Leider können wir deine Anfrage nicht akzeptieren.' 
      });
  ...
  const handleSendMessage = async (recipientId: string) => {
    ...
    try {
      await sendMessage.mutateAsync({
        ...
        includesAddress: includeAddress,
      });
            ...
            <TabsContent value="pending" className="mt-0 space-y-3">
              {pendingLoading ? (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold">{attendee.profile?.display_name}</p>
                        <p className="text-sm text-muted-foreground">@{attendee.profile?.username}</p>
                      </div>
                      <Badge variant="secondary">
                        {attendee.status === 'going' ? 'Zusage' : 'Interessiert'}
                      </Badge>
                    </div>

                    <p className="text-xs text-muted-foreground">
                      Angefragt: {format(new Date(attendee.created_at), 'dd. MMM, HH:mm', { locale: de })}
                    </p>

                    <div className="flex gap-2">
                      <Button
                        onClick={() => handleAccept(attendee.id, attendee.user_id)}
                ...
                <EmptyState 
                  ...
                  description="Noch keine offenen Anfragen für dieses Event."
                />
              )}
            </TabsContent>
            ...
            <TabsContent value="accepted" className="mt-0 space-y-3">
              {acceptedLoading ? (
                  ...
                  >
                    ...
                    {selectedAttendee === attendee.user_id ? (
                      <div className="space-y-3 pt-2 border-t border-border/50">
                        <Textarea
                          placeholder="Nachricht mit Event-Details..."
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            className={includeAddress ? 'bg-primary/20' : ''}
                          >
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            onClick={() => handleSendMessage(attendee.user_id)}
                      ...
                      <Button
                        variant="outline"
                        onClick={() => {
                          setSelectedAttendee(attendee.user_id);
                          setMessageContent(`Hey ${attendee.profile?.display_name}! 🎉\n\nHier die Details für "${eventName}":\n\n📍 Adresse: ${eventAddress}\n\nBis bald!`);
                        }}
                ...
                <EmptyState 
                  ...
                  title="Keine Gäste"
                  description="Noch keine akzeptierten Gäste."
                />
              )}
            </TabsContent>
          </div>
...
function EmptyState({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full glass neon-glow-sm text-muted-foreground">
        {icon}
      </div>
