...
interface FollowerInviteSelectorProps {
  selectedIds: string[];
  onSelectionChange: (ids: string[]) => void;
}

export const FollowerInviteSelector = ({
  selectedIds,
  onSelectionChange,
}: FollowerInviteSelectorProps) => {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-foreground">Follower einladen</h2>
        </div>
      ...
      {selectedIds.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {selectedIds.slice(0, 5).map((id) => {
            const f = followers.find((fl) => fl.id === id);
            if (!f) return null;
            return (
              <Badge
                key={id}
          ...
          {selectedIds.length > 5 && (
            <Badge variant="secondary">+{selectedIds.length - 5} weitere</Badge>
          )}
        </div>
      )}
      ...
      <div className="max-h-48 space-y-1 overflow-y-auto rounded-lg border border-border/50 p-1">
        {isLoading ? (
          <p className="p-4 text-center text-sm text-muted-foreground">Laden...</p>
        ) : filtered.length === 0 ? (
          <p className="p-4 text-center text-sm text-muted-foreground">
            {followers.length === 0 ? 'Noch keine Follower' : 'Keine Ergebnisse'}
          </p>
        ) : (
          filtered.map((follower) => (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-lg p-2 transition hover:bg-muted/50"
            >
              <Checkbox
              ...
              <div className="text-left">
                <p className="text-sm font-medium">{follower.display_name}</p>
