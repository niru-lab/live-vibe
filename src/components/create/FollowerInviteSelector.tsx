...
export const FollowerInviteSelector = ({ selectedIds, onSelectionChange }: FollowerInviteSelectorProps) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative"><MagnifyingGlass weight="thin" className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input data-testid="guest-search-input" placeholder="Follower suchen..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" /></div>
      ...
      {selectedIds.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {selectedIds.slice(0, 5).map((id) => { const f = followers.find((fl) => fl.id === id); if (!f) return null; return (<Badge data-testid="guest-list-item" key={id} variant="default" className="cursor-pointer gap-1 bg-primary/10 text-primary hover:bg-primary/20" onClick={() => toggleFollower(id)}>{f.display_name}<X weight="thin" className="h-3 w-3" /></Badge>); })}
      ...
      <div className="max-h-48 space-y-1 overflow-y-auto rounded-lg border border-border/50 p-1">
        {isLoading ? <p className="p-4 text-center text-sm text-muted-foreground">Laden...</p> : filtered.length === 0 ? (
          ...
          <button data-testid="guest-suggestion" key={follower.id} type="button" onClick={() => toggleFollower(follower.id)} className="flex w-full items-center gap-3 rounded-lg p-2 transition hover:bg-muted/50">
