      ...
      {posts.map((post) => {
        ...
        return (
          <button key={post.id} onClick={() => onPostClick(post)} className="group relative aspect-square overflow-hidden rounded-lg bg-[#12121A] focus:outline-none transition-all duration-300 hover:scale-[1.03] hover:z-10">
            ...
            {isLive && <div className="absolute left-2 top-2 flex items-center gap-1 glass-pill rounded-full px-2 py-1"><div className="h-2 w-2 rounded-full bg-destructive live-pulse" /><span className="text-[10px] font-bold text-white">LIVE</span></div>}
