      ...
      {posts.map((post) => {
        ...
        return (
          <button key={post.id} onClick={() => onPostClick(post)} className="group relative aspect-square overflow-hidden rounded-xl bg-muted focus:outline-none transition-all duration-300 hover:scale-[1.03] hover:z-10">
            ...
            ) : <img src={post.media_url} alt={post.caption || 'Post'} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-110" loading="lazy" onError={(e) => { const img = e.currentTarget; img.style.display = 'none'; img.parentElement?.classList.add('flex', 'items-center', 'justify-center'); const fallback = document.createElement('div'); fallback.className = 'flex items-center justify-center h-full w-full'; fallback.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 256 256" class="text-muted-foreground"><path fill="currentColor" d="M216 40H40a16 16 0 0 0-16 16v144a16 16 0 0 0 16 16h176a16 16 0 0 0 16-16V56a16 16 0 0 0-16-16ZM40 56h176v72.69l-26.34-26.35a16 16 0 0 0-22.63 0L94.34 175l-30.35-30.34a16 16 0 0 0-22.63 0L40 146V56Zm0 144v-17.66l32-32 30.34 30.35a16 16 0 0 0 22.63 0L197.66 108 224 134.34V200Z"/></svg>'; img.parentElement?.appendChild(fallback); }} />
