import { Trophy, TrendingUp, Flame, Clock, Star, Target, ChevronRight } from 'lucide-react';
...
export const AchievementsView = ({
  ...
  const categoryColors: Record<string, string> = {
    ...
    engagement: 'from-orange-500 to-yellow-500',
    performance: 'from-red-500 to-rose-500',
  };
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        ...
        <div className="mt-6 space-y-6">
          ...
          <BadgeCard
            ...
            progressToNext={badgeData.progressToNext}
            nextLevel={badgeData.nextLevel}
            pointsToNext={badgeData.pointsToNext}
          />
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg bg-muted/50 p-3 text-center">
              ...
              <p className="text-xs text-muted-foreground">Social Cloud</p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3 text-center">
              {badgeData.cityRank ? (
                <p className="text-2xl font-bold text-foreground">
                  Top {badgeData.cityRank}
                </p>
              ) : (
                <p className="text-2xl font-bold text-muted-foreground">-</p>
              )}
              <p className="text-xs text-muted-foreground">{city || 'Stadt'}</p>
            </div>
            <div className="rounded-lg bg-muted/50 p-3 text-center">
              <p className="text-2xl font-bold text-foreground">{badgeData.appDays}</p>
          ...
          <div className="flex items-center gap-3 rounded-lg bg-gradient-to-r from-green-500/10 to-emerald-500/10 p-3">
            ...
            <div>
              <p className="font-semibold text-foreground">
                +{badgeData.weeklyGain.toLocaleString()} SC
              </p>
                ...
                return (
                  <div key={key} className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {category.factors.length} Faktoren
                    </p>
                  </div>
                );
              })}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate">{streak.name}</p>
                      {isActive && (
                        <Badge variant="outline" className="h-4 text-[10px] border-green-500 text-green-500">
                          ✓ Aktiv
                        </Badge>
                      )}
                    </div>
            ...
            <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/20 text-2xl">
                  {badgeData.nextLevel.emoji}
                </div>
                <div>
                  <p className="font-semibold text-foreground">
                    Level-Up zu {badgeData.nextLevel.name}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Noch {badgeData.pointsToNext.toLocaleString()} Punkte
                  </p>
                </div>
              ...
              {BADGE_LEVELS.map((badge) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className={cn('text-sm font-medium', badge.color)}>{badge.name}</p>
                    <p className="text-xs text-muted-foreground">
                      {badge.minPoints.toLocaleString()}+ Punkte
                    </p>
                  </div>
                  {badge.level <= badgeData.level && (
