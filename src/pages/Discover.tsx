  ...
  return (
    <AppLayout>
      <div className="fixed inset-0 flex flex-col bg-background">
        <header className="z-40 border-b border-border bg-background/95 backdrop-blur-xl" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
          <div className="space-y-3 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-full glass-pill">
                  <Compass weight="fill" className="h-4 w-4 text-primary" />
                </div>
                <FeyrnLogo size="sm" />
              </div>
              <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full glass-pill" onClick={handleRefresh} disabled={isRefreshing}>
                <ArrowsClockwise weight="thin" className={`h-4 w-4 text-muted-foreground ${isRefreshing ? 'animate-spin' : ''}`} />
              </Button>
            </div>
