        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-muted">
              <Envelope weight="fill" className="h-4 w-4 text-primary" />
            </div>
            <span className="text-lg font-display font-bold text-foreground">Nachrichten</span>
            {unreadCount > 0 && (
              <Badge className="bg-primary text-primary-foreground border-0">{unreadCount} neu</Badge>
            )}
          </div>
        </div>
  ...
  return (
    <div className={cn(cardBase, 'ring-2 ring-primary/30')}>
      <button onClick={onOpen} className="w-full text-left flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <p className="font-semibold text-foreground truncate">{o?.display_name}</p>
            <span className="text-xs text-muted-foreground">@{o?.username}</span>
          </div>
          <p className="text-xs text-muted-foreground mb-2">möchte mit dir schreiben</p>
          <p className="text-sm text-foreground/90 line-clamp-3 break-words">{conv.lastMessage.content}</p>
      ...
      <div className="flex gap-2 mt-3">
        <Button onClick={onAccept} size="sm" className="flex-1 gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90 border-0">
...
function ChatCard({
  ...
  const o = conv.other;
  return (
    <div className={cn(cardBase, conv.unread && 'ring-2 ring-primary/40')}>
      <button onClick={onClick} className="w-full text-left flex items-start gap-3">
        <div className="relative">
          <Avatar className="h-12 w-12 ring-2 ring-primary/40">
            <AvatarImage src={o?.avatar_url || ''} />
            <AvatarFallback className="bg-primary text-primary-foreground">{o?.display_name?.charAt(0)}</AvatarFallback>
          </Avatar>
          <div className="absolute -bottom-1 -right-1 rounded-full p-1 bg-primary">
            {isSent ? <ArrowUp weight="bold" className="h-2.5 w-2.5 text-primary-foreground" /> : <ArrowDown weight="bold" className="h-2.5 w-2.5 text-primary-foreground" />}
          </div>
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <p className="font-semibold text-foreground truncate">{o?.display_name}</p>
            {conv.unread && <div className="h-2 w-2 rounded-full bg-primary animate-pulse" />}
          </div>
          <p className="text-sm text-foreground/80 line-clamp-2 break-words">{conv.lastMessage.content}</p>
          {pendingOutgoing && (
            <p className="text-[11px] text-amber-600 dark:text-amber-400 mt-1">Nachricht gesendet — warte auf Bestätigung</p>
          )}
          {declined && (
            <p className="text-[11px] text-destructive mt-1">Anfrage wurde abgelehnt</p>
          )}
      ...
      <button onClick={onDelete} className="mt-2 text-[11px] text-muted-foreground hover:text-destructive flex items-center gap-1">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !isSent && !message.is_read && 'ring-2 ring-primary/50',
      )}
    >
      <div className="flex items-start gap-3">
        <div className="relative">
          <Avatar className="h-12 w-12 ring-2 ring-primary/40">
            ...
            <AvatarFallback className="bg-primary text-primary-foreground">{person?.display_name?.charAt(0) || '?'}</AvatarFallback>
          </Avatar>
          <div className="absolute -bottom-1 -right-1 rounded-full p-1 bg-primary">
            {isSent ? <ArrowUp weight="bold" className="h-2.5 w-2.5 text-primary-foreground" /> : <ArrowDown weight="bold" className="h-2.5 w-2.5 text-primary-foreground" />}
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="font-semibold text-base truncate text-foreground">{person?.display_name}</p>
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
            ...
            {isSent && message.is_read && <span className="flex items-center gap-1 text-primary">✓✓ Gelesen</span>}
          </div>
        </div>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="font-semibold text-base mb-1 text-foreground">{text}</h3>
      <p className="text-sm text-muted-foreground max-w-xs">Sobald jemand dir schreibt, erscheinen die Nachrichten hier.</p>
    </div>
  );
}
