...
import { useEventAttendees, useUserEventRSVP, useRSVP, useFriendsAttending } from '@/hooks/useEventAttendees';
...
export default function EventDetail() {
  ...
  const handleDeleteEvent = async () => {
    ...
    catch { toast({ variant: 'destructive', title: 'Fehler', description: 'Event konnte nicht gelöscht werden.' }); }
  };

  if (isLoading) return (<AppLayout hideNav><div className="p-4 space-y-4"><Skeleton className="aspect-video w-full rounded-xl" /><Skeleton className="h-8 w-3/4" /><Skeleton className="h-4 w-1/2" /><Skeleton className="h-32 w-full" /></div></AppLayout>);
  if (!event) return (<AppLayout hideNav><div className="flex flex-col items-center justify-center p-8 text-center"><h2 className="text-xl font-bold">Event nicht gefunden</h2><p className="text-muted-foreground mt-2">Das Event existiert nicht mehr.</p><Button onClick={() => navigate('/events')} className="mt-4">Zurück zu Events</Button></div></AppLayout>);
    ...
    <AppLayout hideNav>
        ...
        <div className="flex items-center gap-2">
          ...
          {isCreator && (
            <>
              ...
              <Button variant="outline" size="sm" onClick={() => navigate(`/events/${id}/edit`)}>Bearbeiten</Button>
              <AlertDialog>
                <AlertDialogTrigger asChild><Button variant="outline" size="icon" className="text-destructive hover:bg-destructive/10"><Trash weight="thin" className="h-4 w-4" /></Button></AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader><AlertDialogTitle>Event löschen?</AlertDialogTitle><AlertDialogDescription>Das Event "{event.name}" wird unwiderruflich gelöscht. Alle Zusagen und Nachrichten gehen verloren.</AlertDialogDescription></AlertDialogHeader>
                  <AlertDialogFooter><AlertDialogCancel>Abbrechen</AlertDialogCancel><AlertDialogAction onClick={handleDeleteEvent} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Löschen</AlertDialogAction></AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
            </>
          )}
      ...
      <div className="pb-24">
        <div className="relative aspect-video bg-muted">
          ...
          <div className="absolute right-3 top-3"><Badge variant="secondary" className="bg-background/80 backdrop-blur text-lg px-3 py-1">{event.is_free ? 'Kostenlos' : `${event.entry_price}€`}</Badge></div>
        </div>
        <div className="p-4 space-y-6">
          ...
          <Sheet open={showAttendees} onOpenChange={setShowAttendees}>
            <SheetTrigger asChild>
              <button className="w-full rounded-2xl border border-border/50 bg-card p-4 text-left transition hover:border-primary/50">
                <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><Users weight="thin" className="h-5 w-5 text-primary" /><span className="font-semibold">{goingCount}/{expectedAttendees} zugesagt</span>{fillPercentage > 70 && <Badge variant="secondary" className="bg-orange-500/10 text-orange-500"><Flame weight="thin" className="mr-1 h-3 w-3" />Wird voll!</Badge>}</div><span className="text-sm text-primary">{Math.round(fillPercentage)}%</span></div>
          ...
          <div className="space-y-3 rounded-2xl border border-border/50 bg-card p-4">
            <div className="flex items-start gap-3"><MapPin weight="thin" className="h-5 w-5 text-primary mt-0.5" /><div><p className="font-medium">{event.location_name}</p><p className="text-sm text-muted-foreground">{event.address}, {event.city}</p></div></div>
            <div className="flex items-center gap-3"><CalendarBlank weight="thin" className="h-5 w-5 text-primary" /><p className="font-medium">{format(startsAt, 'EEEE, d. MMMM yyyy', { locale: de })}</p></div>
            <div className="flex items-center gap-3"><Clock weight="thin" className="h-5 w-5 text-primary" /><p className="font-medium">{format(startsAt, 'HH:mm')} Uhr{endsAt && ` – ${format(endsAt, 'HH:mm')} Uhr`}</p></div>
            {!event.is_free && <div className="flex items-center gap-3"><CurrencyEur weight="thin" className="h-5 w-5 text-primary" /><p className="font-medium">{event.entry_price}€ Eintritt</p></div>}
            {event.dresscode && <div className="flex items-center gap-3"><TShirt weight="thin" className="h-5 w-5 text-primary" /><p className="font-medium">{event.dresscode}</p></div>}
          </div>
